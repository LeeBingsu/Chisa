...
- `models/*.zip` — 치사 MMD 모델 본체 (교복 85MB · 수영복 37MB)
...
- 모델은 `models/` 에 올려 두었다. **저장소에 올려 둔 모델 바로 불러오기** 버튼 한 번이면
  세 모델이 모두 뜨고, 다른 zip 은 끌어다 놓으면 된다. 어느 쪽이든 파일은 브라우저 메모리에서만
  blob URL 로 살아 있다가 탭을 닫으면 사라진다.
  이용 조건과 원 배포처는 [`models/README.md`](models/README.md) 참고.
- zip 여러 개를 한 번에 넣으면 안에 든 `.pmx` 를 모두 찾아 **모델 전환 버튼**으로 만든다
  (기본 교복 ↔ 수영복).
- **의상 레이어** — 재질 이름으로 레이어를 추정해 바깥쪽부터 차례로 벗긴다.

  | 단계 | 남는 것 |
  | --- | --- |
  | 전부 입기 | L0~L3 전부 |
  | 액세서리 벗기 | L3 끔 (벨트·리본·장식) |
  | 겉옷 벗기 | L2 끔 (재킷·치마·양말·신발) |
  | 전부 벗기 | L1 까지 끔 — L0 본체(피부·얼굴·머리카락)만 남음 |

  아래 체크박스로 재질 하나하나를 직접 켜고 끌 수도 있어서, 레이어 추정이 빗나간 재질은 손으로 고친다.
  교복 모델은 34개 재질이 L0 21 / L1 1 / L2 8 / L3 4 로, 수영복 모델은 45개가 L0 21 / L1 2 / L2 4 / L3 18 로 잡힌다.
